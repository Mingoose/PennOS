...
static void spawn(bool nohang) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        const int id =
            p_spawn(nap, argv, 1, STDIN_FILENO, STDOUT_FILENO, !nohang);
...
static void spawn_r(void) {
    ...
    if (i < 26) {
        ...
        pid = p_spawn(spawn_r, argv, 1, STDIN_FILENO, STDOUT_FILENO, false);
...
void hang(void) { spawn(false); }

void nohang(void) { spawn(true); }

void recur(void) { spawn_r(); }
